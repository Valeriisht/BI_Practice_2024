...
- Сначала из файла protein_name сохраним только id выравненых послежовательностей

  ```sh
  awk '{print $2}' protein_name | sort | uniq > protein_id.txt
  ```
- Далее получаем аминокислотные последовательности бьелков 
```sh
seqtk subseq augustus.whole.aa protein_id.txt > protein_seq.fasta
```
6.  Предсказание с помощью TargetP Server
...
- Загружае последовательность белков и анализируем результаты работиы программы
